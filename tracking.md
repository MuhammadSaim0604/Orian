...
### The capture fix crashed the app, and the reason was mine

Reported immediately after the round above: tapping **Allow screen capture**, then **Allow** in the system dialog, **crashed the app**.

Two log lines carry the whole story:

```
E MediaProjection unavailable after 10 attempts
  java.lang.SecurityException: Media projections require a foreground service of type
  ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION
...
I Screen capture service running; MediaProjection can now be created
I Screen capture service stopped
E FATAL EXCEPTION: main
  android.app.RemoteServiceException$ForegroundServiceDidNotStartInTimeException:
  Context.startForegroundService() did not then call Service.startForeground()
```

The service started **after** all ten attempts had already failed. That ordering is the bug, and it is not subtle once seen: `startForegroundService` only _posts_ `onStartCommand` to the **main thread**, and `attachScreenCapture` was polling with `Thread.sleep` from `onActivityResult`, which **runs on the main thread**. The polling loop was itself the thing preventing the service from starting. A textbook self-inflicted deadlock, written into the fix for the previous bug.

Then it got worse. The loop gave up, the failure path called `stop`, the queued `onStartCommand` finally ran, and the service entered and left the foreground in the same millisecond. Android had been promised a `startForeground` from that start and treated the immediate stop as the promise being broken — so it killed the process. The crash was **caused by the error handling**, not by the original failure.

The reasoning error worth recording: I treated "asynchronous" as "finishes shortly, so wait for it" when it actually meant "finishes on the thread you are occupying". A poll is only honest when the thing being polled can make progress while you wait.

**The shape now:** `ScreenCaptureService.start` takes a callback and reports when it has genuinely entered the foreground; `attachScreenCapture` is callback-shaped for the same reason, and `onActivityResult` settles its promise from that callback. Nothing blocks. A 3s backstop answers the caller if the service never reports in, and `stop` is called **only** when `isInForeground` is true — stopping an unhonoured start is what killed the process, so it is now guarded rather than merely avoided in one path.
...
## Device-testing pass after Step 4 (all fixed)

Eleven manual tests. **Nine passed outright**, two found real defects, and the user gave a set of UI corrections alongside. Commit `ec43145`; Android CI `33350648512`, TypeScript CI `33350648514`, both green.

Passing: the provider registry and `/models` discovery, manual model entry, two providers with one active, the backgrounded run continuing (the Step 3 fix confirmed on hardware at last), a follow-up building on the previous run's memory, **conversations surviving a force-stop** — which closes the persistence question issue G5 was holding open — a disabled tool no longer being attempted, sessions and their grouping, and screen capture granting without crashing or disturbing the other two rows.

### The AI's messages did not render until the chat was reopened

The user's own words: the prompt bubble renders, then nothing, and pressing back and reopening shows the entire exchange at once.

**The messages were always being stored.** `runController.persistEvent` writes through `sessionStorage.appendMessage`; `sessionStore.messages` was only re-read inside `post()` and `open()`. Nothing told the transcript to look again — and reopening the chat calls `open()`, which is exactly why the workaround worked.

Worth stating because the symptom points the wrong way: this looked like the agent not replying, or the bridge dropping events, and both would have been the wrong thing to investigate. The writes were succeeding the whole time.

Fixed with an `onMessageAppended` notifier in `sessionStorage`, with the store subscribing **at module scope**. Two decisions:

- **The notifier belongs at the storage layer**, because that is the one module both the run controller and the store already import. Having the controller call the store would invert the dependency direction; having the store poll would be guesswork about timing.
- **Module scope, not a component effect.** The run outlives any screen (ADR 0016), so a mount-scoped subscription would miss precisely the messages written while the user was elsewhere — which is most of them.

`post()` no longer re-reads directly, or every message would cost two reads.

### A tool read "On" while its permission was missing

The user found the workaround themselves: switch the tool off and on again, and the permission is requested properly.

The underlying default is right and stays. The stored set is the **disabled** tools, so everything reads On from a fresh install and a newly shipped tool is never silently unavailable. What was wrong was the row saying only "On" when it could not act.

The row now has three states rather than two — off, on, and **on but blocked**. A blocked row takes a warning border, names the missing permission, and offers Grant from the row. The toggle still reflects the user's choice: flipping it off on their behalf would look tidier and would lie about what they set.

### Models are an id and a name

They answer different questions, which is why one string was never enough. The **id** is what the request must carry and belongs to the provider; the **name** is what a person picks out of a list and should be theirs to write.

`StoredModel(id, name)` replaces the bare string. The part that matters is the upgrade path: `decodeModels` still reads the old bare-string form, turning each into a model whose name equals its id — which is exactly what discovery produces for a fresh list, so an upgraded list is indistinguishable from a newly fetched one and nobody opens settings to find it empty. `splitTopLevel` gained depth tracking, because the comma between an object's own two fields would otherwise tear every entry in half.

`mergeModels` keeps **existing** names when discovery runs again. Re-fetching is routine — the cache has a TTL — so a rename lost to tapping Fetch models would be maddening and would look like the app forgetting. Manually entered models survive a discovery that does not list them, since a partial `/models` response is a real possibility.

The chips became two stacked full-width fields per model with a delete icon. Stacked rather than side by side because `gpt-4o-mini-2024-07-18` in half a phone's width ellipsizes to nothing useful, and the id is the part that has to be exactly right. Both fields commit on blur: a per-keystroke write would persist `gpt-4`, `gpt-4o`, `gpt-4o-` on the way to a valid id, and each intermediate value would be a real stored model.

The key field now saves and fetches when it loses focus, because the sequence was otherwise four deliberate steps and the intent after typing a key is obvious. **Fetch models** stays, for asking again later.

### UI corrections

- **A model picker sheet** from the chat header — searchable, grouped by provider, collapsible, animated. Choosing a model **also activates its provider**, because a model belongs to one and selecting one the run would not use makes no sense; that is what makes it one tap. Search overrides a collapsed group, or a match the user just typed would be hidden and read as broken search. Animated with an `Animated.Value` inside a transparent `Modal` rather than `animationType="slide"`, which on Android slides the whole window including the scrim and reads as a screen replacing the chat.
- **Horizontal transitions inside a mode**, with entering a mode still rising from the bottom. Two motions for two kinds of change: vertical means the whole interface is being replaced, horizontal means navigation within one product. Keyed on the route so it restarts per navigation — a boolean would fire once and every later screen would appear without motion. A third of the width plus a fade rather than the full width, which on a phone takes long enough to feel slow.
- **The composer's action moved inside the field** as an icon, and the field is `rounded-xl` rather than a full pill. Send becomes Stop during a run: one control in one place, since a disabled Send beside an active Stop invites tapping the wrong one.
- **The sidebar slides from the left** at 82% width (capped at 360 for tablets), with the remaining strip as a tappable scrim. The visible conversation is what makes dismissal discoverable, which the previous full-width bottom sheet had removed. Two sections: actions, then recent chats.
- **Settings moved from the chat header into the sidebar**, because the header should hold what a person uses mid-conversation.
- **Onboarding opens `PermissionSetupScreen` as a modal**, not `resetOnboarding()` — which would send the user back through welcome and the mode switcher and lose the conversation they were in.
- **A shared icon set** in `packages/ui/src/components/Icon.ts`, drawn from views rather than an icon font or `react-native-svg`. A missing glyph is invisible until a user reports a blank square, which this project has already produced once; an icon font is another asset that can fail in release while working in debug; and a native dependency for arrows is not a trade worth making. Back/Forward/ChevronUp/Down are one chevron primitive at four rotations so they cannot drift apart, and every icon takes a colour rather than choosing one (ADR 0004). Every icon target is padded to 48dp, which the text links were not — so this fixed reachability as well as appearance.

### Two test-only traps

- **A `jest.fn()` created inside a mocked selector is a new reference on every render.** The sheet's mount effect depends on `refresh`, so it re-ran continuously and cleared the search box mid-test. The real store returns stable actions; the mock now does too.
