...
- **Native event payloads are JSON strings and `addAutomationListener` parses them.** Reading a property off an unparsed payload yields `undefined`, which for a status event means every capability reads false — the user sees their permissions spontaneously turn off. Any new event must go through the same wrapper rather than subscribing to the emitter directly.
- **A status object reporting several independent capabilities must read each one independently, at every construction site.** This has now caused three bugs (E1, then `AutomationEventBridge.emitStatusChanged`, then the unparsed payload). Fixing one site does not fix the others, and it always presents as permissions turning themselves off.
